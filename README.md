...
uv run judge-eval validate-config configs/examples/gemma_hosted_4way_free.yaml

# EVOUNA 데이터셋을 로드하고 필터 정책을 적용해 정규화한 뒤, normalized_samples.parquet으로 저장한다.
uv run judge-eval prepare-data configs/examples/gemma_hosted_4way_free.yaml
...
uv run judge-eval run configs/examples/gemma_hosted_4way_free.yaml
...
uv run judge-eval metrics outputs/20260422_gemma_hosted_4way_free

# 계산된 지표 CSV들을 읽어 모델 순위·약점·운영 적합성 요약을 담은 마크다운 리포트와
# Scott's Pi, precision/recall, prompt sensitivity 히트맵 등 시각화 차트를 생성한다.
uv run judge-eval report outputs/20260422_gemma_hosted_4way_free
