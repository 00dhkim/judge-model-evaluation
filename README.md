...
uv run judge-eval validate-config configs/examples/cerebras_hosted_2way_free.yaml

# EVOUNA 데이터셋을 로드하고 필터 정책을 적용해 정규화한 뒤, normalized_samples.parquet으로 저장한다.
uv run judge-eval prepare-data configs/examples/cerebras_hosted_2way_free.yaml
...
uv run judge-eval run configs/examples/cerebras_hosted_2way_free.yaml
...
uv run judge-eval retry-failures configs/examples/cerebras_hosted_2way_free.yaml
...
uv run judge-eval metrics outputs/20260423_cerebras_hosted_2way_free
